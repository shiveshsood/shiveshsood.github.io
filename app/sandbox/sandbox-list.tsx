...
export function SandboxList({ projects }: SandboxListProps) {
  return (
    <>
      <h1 className="font-display text-lg text-neutral-900 mb-2">Sandbox</h1>
      <p className="text-sm text-neutral-700 leading-relaxed mb-8">
        A home for my vibecoded ideas — experimental apps that may someday
        graduate to become full-fledged projects.
      </p>
      <div className="flex flex-col gap-10">
        {projects.map((project) => (
          <div key={project.title}>
            {/* Header row: title + graduated badge + date */}
            <div className="flex justify-between items-start gap-4 mb-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {project.graduated && (
                  <span className="inline-flex items-center gap-1 font-mono text-xs font-medium text-amber-700 bg-amber-100 rounded-md px-2 py-0.5">
                    <span aria-hidden="true">★</span> graduated
                  </span>
                )}
              </div>
